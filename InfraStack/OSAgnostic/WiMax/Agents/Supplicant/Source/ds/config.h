...
#ifndef CONFIG_CTRL_IFACE_CB
#include <grp.h>
#endif /* CONFIG_CTRL_IFACE_CB */
...
struct wpa_config {
	...
	/**
	 * nac_module_dir - CCX NAC module directory
	 */
	char *nac_module_dir;

	/**
	 * wpa_nac_lib - NAC library for wpa_supplicant
	 */
	char *wpa_nac_lib;
	...
	/**
	 * scfg_enabled - Is Wifi Simple Config enabled
	 * 
	 * If there exists a network block which defines the Simple Config 
	 * parameters, then scfg_enabled will be set to 1.  The Simple 
	 * Config network block will be activated if the remaining network 
	 * blocks fail to produce a successful Association
	 */
	int scfg_enabled;
	...
	/*
	 * scfg_cnt - this number is decremented for each scan that 
	 * fails to produce an Association.  When the countdown hits 0, 
	 * Simple Config mode will be enabled on the next scan
	 */
	int scfg_cnt;	
...
#else
struct wpa_config * wpa_config_read(wpa_config_t * config_file);
#endif
...
int wpa_config_set_int(struct wpa_ssid *ssid, const char *var, int value,
		       int line);
...
#else
struct wpa_config * wpa_config_read_file(const char *name);
struct wpa_config * wpa_config_read(swc_config_t *);
#endif

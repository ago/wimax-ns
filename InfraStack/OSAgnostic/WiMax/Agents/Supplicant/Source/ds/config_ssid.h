...
/**
 * struct wpa_simplecfg - Simple Config network configuration data
 *
 * This structure describes the parameters to be used by the Enrollee in the
 * message exchange with the Registrar as part of the Simple Config protocol.
 * 
 */
struct wpa_simplecfg {
	/*
	 * mode - Mode of operation of the protocol - push button or pin
	 */
	int mode;
	
	/*
	 * pin - the 8 digit pin to be known to both the Enrollee and Registrar
	 */
	char pin[8];
	
	/*
	 * pin_type - the type of pin being used, i.e. - user defined or 
	 * machine generated
	 */
	int pin_type;	
	
	/*
	 * countdown - this number will be decremented for each scan that fails
	 * to produce an Association.  When the countdown hits 0, Simple Config
	 * mode will be enabled on the next scan
	 */
	int countdown;
};
...
struct wpa_ssid {
	 ...
	 */
	u8 bssid[ETH_ALEN];
	u8 dummy[2]; /* make sure rest of structure is aligned */
	 ...
	 *
	 * fast_provisioning can be used to enable in-line provisioning of
	 * EAP-FAST credentials (PAC). Following values are supported:
	 * 0 = no in-live provisioning allowed;
	 * 1 = allow anonymous provisioning;
	 * 2 = allow authenticated provisioning (supported only in CCX build);
	 * 3 = allow both anonymous and authenticated provisioning (supported
	 * only in CCX build)
	 */
	char *phase1;
...
#define MAX_WEP_KEY_LEN 16
	 ...
	 * SSID, are allowed. Note: Most drivers do not support this
	 * configuration (they will ignore this parameter).
	 */
	int mixed_cell;
	...
	/**
	 * pending_req_private_key - Pending private key request
	 *
	 * This field should not be set in configuration step. It is only used
	 * internally when control interface is used to request needed
	 * information.
	 */
	int pending_req_private_key;

	/**
	 * pending_req_ca_certificate - Pending CA certificate request
	 *
	 * This field should not be set in configuration step. It is only used
	 * internally when control interface is used to request needed
	 * information.
	 */
	int pending_req_ca_certificate;
	...
	/**
	 * pac_file_machine - File path or blob name for machine PAC entries
	 *
	 * wpa_supplicant will need to be able to create this file and write
	 * updates to it when PAC is being provisioned or refreshed. Full path
	 * to the file should be used since working directory may change when
...
#ifdef EAP_SIMPLE_CONFIG
	/**
	 * simplecfg - The Wifi Simple Config configuration data 
	 * 
	 */
	struct wpa_simplecfg *scfg;
#endif /* EAP_SIMPLE_CONFIG */

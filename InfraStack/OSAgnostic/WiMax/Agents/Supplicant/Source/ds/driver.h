...
struct wpa_driver_ops {
	 ...
	 *	%WPA_ALG_TKIP, %WPA_ALG_CCMP, %WPA_ALG_CCKM_KRK); %WPA_ALG_NONE
	 *	clears the key.
	...
	/**
	 * set_mode - Set client mode (infra/adhoc)
	 * @priv: private driver interface data
	 * @mode: 0 = infrastructure, 1 = adhoc
	 * Returns: 0 on success, -1 on failure
	 */
	...
	/**
	 * set_channel - Set default channel for adhoc mode
	 * @priv: private driver interface data
	 * @channel: Default channel
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_channel)(void *priv, int channel);

	/**
	 * set_ssid - Set SSID for adhoc mode
	 * @priv: private driver interface data
	 * @ssid: SSID
	...
	/**
	 * set_bssid - Set BSSID for adhoc mode
	 * @priv: private driver interface data
	 * @bssid: BSSID
	 * Returns: 0 on success, -1 on failure
	 *
	 * This function is currently only used to clear the BSSID for adhoc
	 * networks, i.e., this is called with 00:00:00:00:00:00 as the BSSID
	 * when requesting the driver to join an adhoc network.
	 */
	int (*set_bssid)(void *priv, const u8 *bssid);

#ifdef __vxworks
	/**
	 * get_random - Return a random value
	 * @priv: private driver interface data
	 * Returns: 32-bit random value
	 *
	 * Note: This is currently used only on VxWorks.
	 */
	u32 (*get_random)(void *priv);
#endif

#ifdef CONFIG_CCX
	/**
	 * ccx_rogue_ap_add - CCX: Add an AP to Rogue AP list
	 * @priv: private driver interface data
	 * @bssid: BSSID of the new Rogue AP
	 * @reason: Reason for the AP being rogue (CCX_ROGUE_AP_* enum)
	 * Returns: 0 on success, -1 on failure
	 *
	 * If the driver implements CCX Rogue AP reporting, it this function
	 * should be implemented. Otherwise, this must be left to %NULL so that
	 * an internal function in wpa_supplicant will be used to take care of
	 * Rogue AP reporting.
	 */
	int (*ccx_rogue_ap_add)(void *priv, const u8 *addr, int reason);

	/**
	 * ccx_rogue_ap_remove - CCX: Remove an AP from Rogue AP list
	 * @priv: private driver interface data
	 * @addr: BSSID of the AP to remove
	 * Returns: 0 on success, -1 on failure
	 *
	 * If the driver implements CCX Rogue AP reporting, it this function
	 * should be implemented. Otherwise, this must be left to %NULL so that
	 ...
	 * Rogue AP reporting.
	 */
	int (*ccx_rogue_ap_remove)(void *priv, const u8 *addr);

	/**
	 * ccx_rogue_ap_remove_all - CCX: Remove all APs from Rogue AP list
	 * @priv: private driver interface data
	 * Returns: 0 on success, -1 on failure
	 *
	 * If the driver implements CCX Rogue AP reporting, it this function
	 ...
	 * Rogue AP reporting.
	 */
	int (*ccx_rogue_ap_remove_all)(void *priv);

	/**
	 * ccx_rogue_ap_report - CCX: Report Rogue APs to the current AP
	 * @priv: private driver interface data
	 * Returns: 0 on success, -1 on failure
	 *
	 * This function is called when authentication has been completed
	 * successfully with an AP. At this point, the current list of
	 * Rogue APs is sent to the AP. This is only used with LEAP.
	 *
	 * If the driver implements CCX Rogue AP reporting, it this function
	 * should be implemented. Otherwise, this must be left to %NULL so that
	 * an internal function in wpa_supplicant will be used to take care of
	 * Rogue AP reporting.
	 */
	int (*ccx_rogue_ap_report)(void *priv);
#endif /* CONFIG_CCX */

	/**
	 * set_mixed_cell - Enable/disable mixed cell
	 * @priv: private driver interface data
	 * @enabled: Mixed cell enabled (1) or disabled (0)
	 * Returns: 0 on success, -1 on failure
	 *
	 * This function is used to enable/disable mixed cell operations, i.e.,
	 * to allow/disallow association with networks that do not report a
	 * matching security polucy in beacon frames (e.g., due to multi-SSID
	 * configuration).
	 */
	int (*set_mixed_cell)(void *priv, int enabled);
};

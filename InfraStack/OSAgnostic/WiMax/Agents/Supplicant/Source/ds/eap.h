...
/**
 * enum eap_bool_var - EAPOL boolean state variables for EAP state machine
 *
...
enum eap_bool_var {
	/**
	 * EAP_eapSuccess - EAP SUCCESS state reached
	 *
	...
	/**
	 * EAP_eapRestart - Lower layer request to restart authentication
	 *
	...
	/**
	 * EAP_eapFail - EAP FAILURE state reached
	 *
	...
	/**
	 * EAP_eapNoResp - Request has been process; no response to send
	 *
	 * Set to TRUE in EAP state machine, FALSE in lower layer.
	 */
	EAP_eapNoResp,
	...
	/**
	 * EAP_eapReq - EAP request available from lower layer
	 *
	 * Set to TRUE in lower layer, FALSE in EAP state machine.
	 */
	EAP_eapReq,
	...
	/**
	 * EAP_portEnabled - Lower layer is ready for communication
	 *
	...
	/**
	 * EAP_altAccept - Alternate indication of success (RFC3748)
	 *
	 * EAP state machines reads this value.
	 */
	EAP_altAccept,
	...
	/**
	 * EAP_altReject - Alternate indication of failure (RFC3748)
	 *
	 * EAP state machines reads this value.
	 */
	EAP_altReject
...
/**
 * enum eap_int_var - EAPOL integer state variables for EAP state machine
 *
 * These variables are used in the interface between EAP peer state machine and
 * lower layer. These are defined in RFC 4137, Sect. 4.1. Lower layer code is
 * expected to maintain these variables and register a callback functions for
...
enum eap_int_var {
	/**
	 * EAP_idleWhile - Outside time for EAP peer timeout
	 *
...
struct eapol_callbacks {
	 ...
	 */
	Boolean (*get_bool)(void *ctx, enum eap_bool_var variable);
	 ...
	 */
	void (*set_bool)(void *ctx, enum eap_bool_var variable,
			 Boolean value);
	 ...
	 * Returns: Value of the EAPOL variable
	 */
	unsigned int (*get_int)(void *ctx, enum eap_int_var variable);
	 ...
	 * @value: Value for the EAPOL variable
	 */
	void (*set_int)(void *ctx, enum eap_int_var variable,
			unsigned int value);
...
void eap_sm_request_private_key(struct eap_sm *sm, struct wpa_ssid *config);
void eap_sm_notify_ctrl_attached(struct eap_sm *sm);
void eap_sm_notify_ctrl_response(struct eap_sm *sm);
